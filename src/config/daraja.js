...
router.get("/api/access_token", (_req, res) => {
  getAccessToken()
    .then((accessToken) => {
...
async function getAccessToken() {
  const consumerKey = "ODAMbqilk86oYg2JMQTVYsZkhWJsThARwhGXC04Ka0aYeSGJ";
  const consumerSecret =
    "lGFg2pnYT56bn0vS3Pfd0cFmiUYU7OSA01TUnWZxe16NLAwbrnsaqJKswR68nxuL";
  const url =
...
router.post("/api/callback", (req, res) => {
  ...
  writeFile("stkcallback.json", json, "utf8", function (err) {
    if (err) {
